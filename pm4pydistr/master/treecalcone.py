...
from pm4pydistr.master.child_dfg import ChildDFG

class SubtreeDFGBasedOne():
    def __init__(self, dfg, master_dfg, initial_dfg, activities, counts, rec_depth, conf, noise_threshold=0,
        ...
        if self.dfg:
            ...
            if xor_cut[0]:
                ...
                for i, comp in enumerate(xor_cut[1]):
                    ...
                    ChildDFG(new_dfg, self.master_dfg, self.initial_dfg, comp, self.counts, self.rec_depth + 1,
                ...
                if seq_cut[0]:
                    ...
                    for i, child in enumerate(seq_cut[1]):
                        ...
                        ChildDFG(dfg_child, self.master_dfg, self.initial_dfg, child, self.counts, self.rec_depth + 1,
                                 self.noise_threshold, self.initial_start_activities, self.initial_end_activities,
                        ...
                        if loop_cut[0]:
                            if loop_cut[2]:
                                ...
                                for index_enum, child in enumerate(loop_cut[1]):
                                    dfg_child = filter_dfg_on_act(self.dfg, child)
                                    ChildDFG(dfg_child, self.master_dfg, self.initial_dfg, child, self.counts,
                                             self.rec_depth + 1,
                                             self.noise_threshold, self.initial_start_activities,
                                             self.initial_end_activities,
                                             self.detected_cut, index_enum, loop_cut[3], self.conf)
                                ...
                                for index_enum, child in enumerate(loop_cut[1]):
                                    ...
                                    ChildDFG(dfg_child, self.master_dfg, self.initial_dfg, child, self.counts,
                                             ...
                                             self.noise_threshold, self.initial_start_activities,
                                             self.initial_end_activities,
                                             self.detected_cut, index_enum, True, self.conf)
                                    self.child_names.append(index_enum)
                                #     next_subtree = SubtreeDFGBasedOne(dfg_child, self.master_dfg, self.initial_dfg,
                                #                                       child,
                                #                                       self.counts, self.rec_depth + 1,
